...
class ShippingActivity : DrawerBaseActivity() {
    ...
    private fun initCarriers(carrierResponse: MutableList<CarrierData>) {
    ...
    private fun saveShipping() {
        if (binding.etTrackingNumber.text.toString().isNotEmpty() && carrier.id != 0) {
                ...
                if (fillOrders != null) {
                    sendShipping(
                        ShipRequest(
                            ...
                            carrier.id,
                            carrier.name,
                            binding.etTrackingNumber.text.toString()
                        )
                    )
                }
            }
            ...
            messageDialog.showDialog(
                ...
                "Please input a tracking number and carrier"
            ) { }
        }
    }
